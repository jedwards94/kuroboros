...
- `generate deployment`: Generates deployment manifests in `./config/base/deployment`.  
  The `[image]` section in your config sets the image:
...
If you need more controll over the build process use the `docker` command directly or any other OCI Image Builder.

**Options:**
- `--build-arg TEXT` Build arguments for Docker (`key=val`). Can be specified multiple times.
- `--quiet` Hide build output.

**Usage:**
```sh
kuroboros build --build-arg key=val --build-arg key2=val2
```
